...
struct PluginLoader::Impl
{
    ...
    Impl(Http::Server& server, std::map<std::string, std::shared_ptr<Plugin>>& plugins) :
    ...
    void Scan() {
        for (auto& plugin : plugins)
        {
            ...
            {
                const auto lastModificationTime =
                    plugin.second->pluginImageFile.GetLastModifiedTime();
                if (plugin.second->lastModifiedTime != lastModificationTime)
                {
                    diagnosticMessageDelegate(
                        "PluginLoader", 0,
                        StringUtils::sprintf("plugin '%s' appears to have changed",
                                             plugin.first.c_str()));
            ...
            if (plugin.second->needsToLoad)
            {
                plugin.second->Load(server, plugin.second->needsToLoad, pluginsRunTimePath,
                                    diagnosticMessageDelegate);
                if ((plugin.second->unloadDelegate == nullptr) && plugin.second->needsToLoad)
                {
                    diagnosticMessageDelegate(
                        "PluginLoader", SystemUtils::DiagnosticsSender::Levels::WARNING,
                        StringUtils::sprintf("plugin '%s' failed to copy... will attempt "
                                             "to copy and reload soon",
                                             plugin.first.c_str()));
            ...
            } else
            {
                diagnosticMessageDelegate(
                    ...
                    StringUtils::sprintf("unable to find plugin image '%s' file",
                                         plugin.first.c_str()));
                pluginLoaderScan = true;
            }
        }
    ...
    void Launch() {
        ...
        while (!pluginLoaderStop)
        {
            ...
            (void)pluginLoaderWakeCondition.wait(
                lock, [this] { return pluginLoaderScan || pluginLoaderStop; });
            ...
            if (pluginLoaderScan)
            {
                ...
                if (pluginLoaderWakeCondition.wait_for(
                        lock, std::chrono::microseconds(100),
                        [this] { return pluginLoaderScan || pluginLoaderStop; }))
                {
                    diagnosticMessageDelegate("PluginLoader", 0,
                                              "Need to scan ... still updating; backing off");
                } else
                {
...
PluginLoader::PluginLoader(
    Http::Server& server, std::string pluginRuntimePath, std::string pluginsImagePath,
    std::map<std::string, std::shared_ptr<Plugin>>& plugins,
    SystemUtils::DiagnosticsSender::DiagnosticMessageDelegate diagnosticMessageDelegate) :
...
void PluginLoader::StartScanning() {
    ...
    if (!impl_->directroyMonitor.Start(imagePathChangedDelegate, impl_->pluginsImagePath))
    {
